...
Simulador de provas Microsoft Azure Fundamentals (AZ-900) com Flask.
...
Cada resposta e gravada em `history_az900.csv` (pergunta, alternativas, correta,
...
HISTORY_CSV = os.path.join(BASE_DIR, "history_az900.csv")
...
# MUDAR LINK
CERT_URL = "https://learn.microsoft.com/en-us/credentials/certifications/resources/study-guides/az-900"
...
def topicos_menos_vistos():
    contagem = {t: 0 for t in TOPICOS_AZ900}
    if os.path.isfile(HISTORY_CSV):
        try:  # ← adicionar
            with open(HISTORY_CSV, "r", newline="", encoding="utf-8-sig") as f:
                for linha in csv.DictReader(f):
                    topico = linha.get("topico", "")
                    for t in TOPICOS_AZ900:
                        if topico in t:
                            contagem[t] += 1
        except Exception as e:
            print(f"[HIST] Erro ao calcular tópicos menos vistos ({e}). Usando amostra aleatória.")
            return random.sample(TOPICOS_AZ900, k=5)  # fallback seguro
...
def montar_prompt(evitar, contexto, usando_web=False):
    ...
    system = (
        ...
        "realistas para o exame AZ-900. As questoes "
        "devem refletir o edital atual: focadas fortemente em ... "
        "As questoes devem ser conceituais. Evite pegadinhas com numeros ou decoreba; "
        " Responda SEMPRE em inglês e SEMPRE em JSON valido."
    )

    user = f"""Gere EXATAMENTE {QUESTIONS_PER_BATCH} questoes de multipla escolha para o exame AZ-900.
...
- Distribua as questões em 3 níveis: 1 fácil, 2 intermediárias, 2 difíceis.
...
{{
  "questions": [
    {{
      "topico": "string (um dos dominios do AZ-900)",
      "pergunta": "string",
...
def gerar_perguntas():
    ...
    if not contexto:
        consulta_rag = " ".join(random.sample(TOPICOS_AZ900, k=min(3, len(TOPICOS_AZ900))))
