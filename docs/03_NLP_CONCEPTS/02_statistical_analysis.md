...
After tokenizing, normalizing, and applying lemmatization to the text, the frequency of each term can be counted and tabulated
...
A high TF-IDF score indicates that a word appears often in one document but rarely in others. A low score indicates that word is common in many documents. In two samples about AI agents, because "AI", "Microsoft", and "agent" appear in both samples (N = 2, df(t) = 2), their IDF is log(2/2) = 0, so they carry no discriminative weight in TF‑IDF.
...
Next, edges are created between sentences with weights based on similarity (word overlap).
